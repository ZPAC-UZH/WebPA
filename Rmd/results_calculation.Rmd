...
rating_input_df <- 
  file.path(path_input, "submitted_ratings") %>% 
...
```{r}
# calculate fudge factor based on the number of submissions
n_submitted_df <- 
  rating_input_df %>% 
...
webpa_df <- 
  ...
  # generate list of all students as rater x ratee
  students_group_df %>%
  ...
  # normalize by total given per rater
  group_by(group_number, rater) %>%
  mutate(rater_total_given = sum(rating)) %>%
  ungroup() %>%
  mutate(norm_received = rating / rater_total_given) %>% 

  # handle the case where some member did not submit anything
  mutate(norm_received = if_else(is.nan(norm_received), 0, norm_received)) %>% 
  
  # accumulate normalized rating by ratee
  group_by(group_number, ratee) %>% 
  summarize(sum_norm_received = sum(norm_received)) %>% 
  ungroup() %>% 
  ...
  # handle the case where nobody submitted
